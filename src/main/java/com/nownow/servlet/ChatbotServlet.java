...
public class ChatbotServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        root.add("messages",            buildMessages(req, history));
     ...
     */
    private JsonArray buildMessages(HttpServletRequest req, JsonArray history) {
        JsonArray messages = new JsonArray();

        JsonObject systemMsg = new JsonObject();
        systemMsg.addProperty("role",    "system");
        systemMsg.addProperty("content", buildSystemPrompt(req));
    ...
    /** Role-aware system prompt — adapts to Customer / Driver / Admin. */
    private String buildSystemPrompt(HttpServletRequest req) {
        ...
        if (user != null) {
            ...
            switch (user.getRole()) {
                case CUSTOMER -> sb.append(
                    "Help them track packages, submit new deliveries, and understand statuses.");
                case DRIVER   -> sb.append(
                    "Help them manage assigned deliveries, mark pickups/deliveries, and report failures.");
                case ADMIN    -> sb.append(
                    "Help them assign drivers, edit/delete packages, and interpret the 5 business reports.");
            }
        } else {
