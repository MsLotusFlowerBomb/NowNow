...
public class PackageDAO {
  ...
  public void updateDetails(int packageId, String recipientName,
      String deliveryAddress, String description) throws SQLException {
    String sql = "UPDATE packages SET recipient_name = ?, delivery_address = ?, "
  ...
  /**
   * Deletes a package and its tracking events (admin use).
   * Tracking events are deleted first to respect the FK constraint.
   */
  public void delete(int packageId) throws SQLException {
    // Delete child tracking events first (FK constraint)
    String deleteEvents = "DELETE FROM tracking_events WHERE package_id = ?";
    String deleteDelivery = "DELETE FROM deliveries WHERE package_id = ?";
    String deletePkg    = "DELETE FROM packages WHERE id = ?";

    try (Connection conn = DBConnection.getConnection()) {
      conn.setAutoCommit(false);
      try {
        try (PreparedStatement ps = conn.prepareStatement(deleteEvents)) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(deletePkg)) {
          ps.setInt(1, packageId);
          ps.executeUpdate();
        }
        conn.commit();
