...
public class DeliveryDAO {
	...
	// -------------------------------------------------------
	public void completeDelivery(int deliveryId, int packageId, int driverId) throws SQLException {
		String sql = "{call CompleteDelivery(?, ?, ?)}";
		try (Connection conn = DBConnection.getConnection(); CallableStatement cs = conn.prepareCall(sql)) {
			cs.setInt(1, deliveryId);
			cs.setInt(2, packageId);
			cs.setInt(3, driverId);
			cs.execute();
		}
	}
	...
	public void assignDriver(int packageId, int driverId) throws SQLException {
		String sql = "{call AssignDriverToPackage(?, ?)}";
		try (Connection conn = DBConnection.getConnection(); CallableStatement cs = conn.prepareCall(sql)) {
			cs.setInt(1, packageId);
			cs.setInt(2, driverId);
			cs.execute();
		}
	}
	...
	private Delivery mapRow(ResultSet rs) throws SQLException {
		...
		if (assigned != null) {
			d.setAssignedAt(assigned.toLocalDateTime());
		}
		Timestamp pickedUp = rs.getTimestamp("picked_up_at");
		if (pickedUp != null) {
			d.setPickedUpAt(pickedUp.toLocalDateTime());
		}
		Timestamp delivered = rs.getTimestamp("delivered_at");
		if (delivered != null) {
			d.setDeliveredAt(delivered.toLocalDateTime());
		}
		return d;
	}
}
