...
<head>
    ...
    <title>Revenue Report – NowNow</title>
...
<main class="page-main">
    <div class="dashboard-header">
        <div>
            ...
            <p class="dashboard-sub">Estimated revenue from packages for ${rangeLabel}</p>
        </div>
    ...
    <%-- Filters --%>
    <section class="form-card" style="max-width:800px;margin-bottom:1.5rem">
        <h2>Filters</h2>
        <p class="dashboard-sub">Filter by date range and/or package status.</p>
        <form action="${pageContext.request.contextPath}/admin/reports/revenue" method="get">
    ...
    <div class="stats-row">
        <div class="stat-card">
            ...
            <span class="stat-card-label">Total Estimated Revenue</span>
        </div>
        <div class="stat-card">
            <span class="stat-card-number">R <fmt:formatNumber value="${confirmedRevenue}" pattern="#,##0.00"/></span>
            <span class="stat-card-label">Confirmed (Delivered)</span>
        </div>
        <div class="stat-card">
            <span class="stat-card-number">R <fmt:formatNumber value="${pendingRevenue}" pattern="#,##0.00"/></span>
            <span class="stat-card-label">Pending / In Progress</span>
        </div>
        <div class="stat-card">
            <span class="stat-card-number">R <fmt:formatNumber value="${avgRevenue}" pattern="#,##0.00"/></span>
            <span class="stat-card-label">Avg. per Delivered Package</span>
        </div>
        <div class="stat-card">
            <span class="stat-card-number">${totalPackages}</span>
            <span class="stat-card-label">Packages in Range</span>
        </div>
        <div class="stat-card">
        ...
        <c:choose>
            <c:when test="${empty packages}">
                <div class="empty-state">
                    <p>No packages found for the selected filters. Try adjusting the date range or status.</p>
                </div>
            </c:when>
            <c:otherwise>
                        ...
                        <tbody>
                            <c:forEach var="pkg" items="${packages}">
                                <tr>
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${pkg.estimatedPrice != null}">
                                                R <fmt:formatNumber value="${pkg.estimatedPrice}" pattern="#,##0.00"/>
                                            </c:when>
...
<script>
    ...
    function exportCSV() {
        var d = getTableData();
        var lines = [
            '"NowNow – Revenue Report"',
            '"${rangeLabel}"',
            '"Total Estimated Revenue","R ${totalRevenue}"',
            '"Confirmed Revenue (Delivered)","R ${confirmedRevenue}"',
            '"Pending Revenue","R ${pendingRevenue}"',
            '"Average per Delivery","R ${avgRevenue}"',
            ''
        ];
        ...
        a.download = 'revenue_report_'+today()+'.csv';
        a.click();
    }
    ...
    function exportPDF() {
        ...
        doc.text('NowNow – Revenue Report', m, y); y += 7;
        doc.setFontSize(9); doc.setFont('helvetica','normal'); doc.setTextColor(100);
        doc.text('${rangeLabel}', m, y); y += 6;
        doc.text('Total Estimated: R ${totalRevenue}   |   Confirmed: R ${confirmedRevenue}   |   Pending: R ${pendingRevenue}   |   Avg/Delivery: R ${avgRevenue}', m, y); y += 8;
        doc.setTextColor(0);
        if (d.headers.length) {
        ...
        doc.save('revenue_report_'+today()+'.pdf');
    }
</script>
</body>
</html>
